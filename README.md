...
3. Run the kubeadm init setup to setup the first master.
	...
	- Run:
		...
		To make sure that there is one master and the pods are running well. If the node is NotReady (it should be Ready), wait for some time
		so that it is Ready. Same with the pods as well, make sure that the pods are running well without any issues before initing the other
		master's.

		If after waiting for few minutes, the master is still NotReady or the pods are restarting or not all are in Running state, something is
		wrong. Try restarting the kubelet (sudo service kubelet restart). If this does not solve the problem, look for troubleshooting steps.
	...
	- Switch to the second & third & fourth .... master node and perform the instructions below.
